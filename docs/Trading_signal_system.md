...
**“Pocket”**이란 보유 종목, 수량, 매입가 등을 포함하는 거래 단위를 말한다.

**“Strategy(전략)”**란 Trailing Stop, Take Profit 등 Pocket에 부착되는 정책 객체를 말한다.
...
① 각 Pocket은 단일 책임 원칙(SRP)에 따라 독립 객체로 관리한다.
② 각 전략은 별도 Class로 구현하고 Pocket에 부착할 수 있어야 한다.
③ 전략 예시는 다음과 같다.
...
④ Agent 재시작 시 Pocket 및 전략 설정은 복구되어야 한다.
