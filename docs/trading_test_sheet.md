...
| **B-03** | **매수 체결 (가상)**| 1. 지정가 매수 주문 생성.<br>2. 시장가 하락 < 지정가 시뮬레이션. | - 주문 상태 변경: `wait` -> `done`.<br>- **포지션 생성됨** (`PocketManager`).<br>- KRW 잔고 감소, BTC 잔고 증가. | ✅ |
...
| **S-02** | **매도 체결 (가상)**| 1. 지정가 매도 주문 생성.<br>2. 시장가 상승 > 지정가 시뮬레이션. | - 주문 상태 변경: `wait` -> `done`.<br>- **포지션 종료** (부분/전체) (`PocketManager`).<br>- BTC 잔고 감소, KRW 잔고 증가. | ✅ |
...
**참고:** 가상 계좌 로직(`DBTradeManager`)이 수정되어 `myOrder` 이벤트가 모든 상태 변경(`wait`, `done`, `cancel`)에 대해 정상적으로 발생하며, 이를 통해 대시보드와 포지션 로직이 올바르게 업데이트됨을 확인했습니다.
